...
public class AppDraft7 extends JFrame {
    ...
    private JPanel createTenantsScreen() {
        ...
        btnOpenModal.setBackground(Color.WHITE);
        ...
        btnDeleteSelection.setBackground(Color.WHITE);
        btnDeleteSelection.setForeground(Color.BLACK);
        btnDeleteSelection.setFocusPainted(false);

        JButton btnEditSelection = new JButton("Edit Selected");
        btnEditSelection.setFont(new Font("Arial", Font.BOLD, 16));
        btnEditSelection.setBackground(Color.WHITE);
        btnEditSelection.setForeground(Color.BLACK);
        btnEditSelection.setFocusPainted(false);
        ...
        btnEditSelection.addActionListener(e -> {
            int selectedIndex = tenantList.getSelectedIndex();
            if (selectedIndex != -1) {
                String selectedValue = tenantList.getSelectedValue();
                if (selectedValue != null && !selectedValue.trim().isEmpty() && selectedValue.contains("Tenant Name:")) {
                    showEditTenantDialog(selectedIndex, selectedValue);
                } else {
                    JOptionPane.showMessageDialog(this, "Please select a valid tenant entry from the list to edit", "Notice", JOptionPane.WARNING_MESSAGE);
                }
            } else {
                JOptionPane.showMessageDialog(this, "Please select a tenant from the list to edit", "Notice", JOptionPane.WARNING_MESSAGE);
            }
        });

        actionButtonPanel.add(btnOpenModal);
        actionButtonPanel.add(btnDeleteSelection);
        actionButtonPanel.add(btnEditSelection);
    ...
    private void showEditTenantDialog(int index, String data) {

        JDialog dialog = new JDialog(this, "Edit Tenant", true);
        ...
        try {
            ...
            txtName.setText(parts[0].replace("Tenant Name:", "").trim());
            txtRoom.setText(parts[1].replace("Room:", "").trim());
            txtContact.setText(parts[2].replace("Contact:", "").trim());
            txtLease.setText(parts[3].replace("Lease Start:", "").trim());
        ...
        form.add(new JLabel("Lease Start"));
        form.add(txtLease);
        ...
        btnSave.addActionListener(ev -> {
            ...
            String updated =
                    "Tenant Name: " + txtName.getText().trim()
                    + "  |  Room: " + txtRoom.getText().trim()
                    + "  |  Contact: " + txtContact.getText().trim()
                    + "  |  Lease Start: " + txtLease.getText().trim();

            listModel.set(index, updated);

            saveAllTenantsToFile();
    ...
    private void saveAllTenantsToFile() {
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(FILE_PATH, false))) {
            ...
            for (int i = 0; i < listModel.size(); i++) {
                ...
                String name =
                        parts[0].replace("Tenant Name:", "").trim();

                String room =
                        parts[1].replace("Room:", "").trim();

                String contact =
                        parts[2].replace("Contact:", "").trim();

                String lease =
                        parts[3].replace("Lease Start:", "").trim();
    ...
    private JPanel createUtilitiesScreen() {
        ...
        btnEditSelection.addActionListener(e -> {
            int selectedIndex = utilityList.getSelectedIndex();
            if (selectedIndex != -1) {
                String selectedValue = utilityList.getSelectedValue();
                if (selectedValue != null && !selectedValue.trim().isEmpty() && selectedValue.contains("Status:")) {
                    showEditUtilityDialog(selectedIndex);
                } else {
                    JOptionPane.showMessageDialog(this, "Please select a valid utility entry from the list to edit", "Notice", JOptionPane.WARNING_MESSAGE);
                }
            } else {
                JOptionPane.showMessageDialog(this, "Please select a utility from the list to edit", "Notice", JOptionPane.WARNING_MESSAGE);
            }
        });
    ...
    private void showEditUtilityDialog(int selectedIndex) {

        String selectedData =
                utilityListModel.get(selectedIndex);
        ...
        String tenant =
                parts[1].replace("Tenant:", "").trim();

        String electricity =
                parts[2].replace("Electricity:", "").trim();

        String water =
                parts[3].replace("Water:", "").trim();

        String other =
                parts[4].replace("Other:", "").trim();

        JDialog dialog =
                new JDialog(this, "Edit Utility", true);
        ...
        JTextField txtTenant =
                new JTextField(tenant);

        JTextField txtElectricity =
                new JTextField(electricity);

        JTextField txtWater =
                new JTextField(water);

        JTextField txtOther =
                new JTextField(other);

        formGrid.add(new JLabel("Status:"));
        formGrid.add(cmbStatus);

        formGrid.add(new JLabel("Tenant:"));
        formGrid.add(txtTenant);

        formGrid.add(new JLabel("Electricity:"));
        formGrid.add(txtElectricity);

        formGrid.add(new JLabel("Water:"));
        formGrid.add(txtWater);

        formGrid.add(new JLabel("Other:"));
        formGrid.add(txtOther);
        ...
        btnSave.addActionListener(e -> {
            ...
            String updatedRecord =
                    ...
                    + "  |  Tenant: " + txtTenant.getText().trim()
                    + "  |  Electricity: " + txtElectricity.getText().trim()
                    + "  |  Water: " + txtWater.getText().trim()
                    + "  |  Other: " + txtOther.getText().trim();

            utilityListModel.set(
                    selectedIndex,
                    updatedRecord);
    ...
    private void rewriteUtilityFile() {

        try (BufferedWriter bw =
                    new BufferedWriter(
                            new FileWriter(UTILITIES_FILE, false))) {

            for (int i = 0; i < utilityListModel.size(); i++) {

                String item = utilityListModel.get(i);
                ...
                bw.write("Tenant: " + tenant);
                bw.newLine();

                bw.write("Electricity: " + electricity);
                bw.newLine();

                bw.write("Water: " + water);
                bw.newLine();
